  ...
  getPopulation: async (selectedProvinces: Province[]) => {
    try {
      const data = await Promise.all(
        selectedProvinces.map(async (province: Province) => {
          const res = await axiosResas.get(
            `https://opendata.resas-portal.go.jp/api/v1/population/composition/perYear?prefCode=${province.prefCode}`
          );
          if (
            res.status === 200 &&
            res.data.result &&
            ...
            const populationAndYears = populationAndYearsFromApi.filter(
              (population: Population) => YEARS.includes(population.year)
            );
            const populations = populationAndYears.map(
              (populationAndYear: Population) => populationAndYear.value
            );
            return {
